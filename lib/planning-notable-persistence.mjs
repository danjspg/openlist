...
export function buildDeterministicNotableMutation(
  ...
  for (const category of [...result.categories, ...(result.publicCategories || [])]) {
  ...
  if (result.notable) {
    reasons.deterministic = {
      ...
      publicCategories: result.publicCategories || [],
  ...
  const eligibility = evaluatePlanningNotableEligibility(application, existing, {
    ...
    residentialUnits: result.signals.residentialUnits,
  })
  ...
  const row = {
    ...
    extracted_residential_units: result.signals.residentialUnits || null,
  ...
  const changed = !existing
    ...
    || Number(existing.extracted_residential_units || 0) !== Number(row.extracted_residential_units || 0)
...
const NOTABLE_SELECT = "application_id,source,reason,evidence,active,priority_eligible,extracted_residential_units,display_name,search_aliases,notable_categories,classification_reasons,classification_sources"
