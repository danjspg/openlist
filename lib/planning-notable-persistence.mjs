...
export function buildDeterministicNotableMutation(application, existing, classification) {
  ...
  return { changed, row, classification: result }
}
...
const NOTABLE_SELECT = "application_id,source,reason,evidence,active,display_name,search_aliases,notable_categories,classification_reasons,classification_sources"

export async function classifyAndPersistPlanningApplications(
  supabase,
  applications,
  { dryRun = false, enqueue = true, now = () => new Date().toISOString() } = {}
) {
  ...
  const results = rows.map((application) => {
    ...
    return {
      application,
      existing,
      ...buildDeterministicNotableMutation(application, existing),
    }
  })
