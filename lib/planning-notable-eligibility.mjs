...
const OUTCOME_DATE_FIELDS = Object.freeze([
  ...
  "latest_lifecycle_event_date",
  "latest_meaningful_event_date",
])

export function planningNotableRetentionMonths(residentialUnits) {
  const units = Number(residentialUnits) || 0
  if (units >= 100) return 60
  if (units >= 50) return 24
  return DEFAULT_PLANNING_NOTABLE_RETENTION_MONTHS
}
...
export function evaluatePlanningNotableEligibility(
  ...
  const effectiveRetentionMonths = residentialUnits >= 10
    ? planningNotableRetentionMonths(residentialUnits)
    : retentionMonths
  const cutoffDate = planningNotableRetentionCutoff(asOf, effectiveRetentionMonths)
  ...
  return {
    ...
    latestOutcomeDate,
    cutoffDate,
    retentionMonths: Math.max(1, Math.floor(Number(effectiveRetentionMonths) || DEFAULT_PLANNING_NOTABLE_RETENTION_MONTHS)),
    overrideSources,
  }
}
