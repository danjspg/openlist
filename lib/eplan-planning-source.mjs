...
const EPLAN_AUTHORITIES = {
  CARLOW: { path: "CarlowCC" },
  CAVAN: { path: "CavanCC" },
  CLARE: { path: "ClareCC" },
  DONEGAL: { path: "DonegalCC" },
  GALWAYCOCO: { path: "GalwayCC" },
  GALWAYCITY: { path: "GalwayCity" },
  KILDARE: { path: "KildareCC" },
  KILKENNY: { path: "KilkennyCC" },
  KERRY: { path: "KerryCC" },
  LAOIS: { path: "LaoisCC" },
  LIMERICK: { path: "LimerickCCC" },
  LEITRIM: { path: "LeitrimCC" },
  LONGFORD: { path: "LongfordCC" },
  LOUTH: { path: "LouthCC" },
  MAYO: { path: "MayoCC" },
  MEATH: { path: "MeathCC" },
  MONAGHAN: { path: "MonaghanCC" },
  WATERFORD: { path: "WaterfordCCC" },
  OFFALY: { path: "OffalyCC" },
  ROSCOMMON: { path: "RoscommonCC" },
  SLIGO: { path: "SligoCC" },
  TIPPERARY: { path: "TipperaryCC" },
  WESTMEATH: { path: "WestmeathCC" },
  WICKLOW: { path: "WicklowCC" },
}
...
function textFor(fields, label) {
  return htmlText(fields.get(label)) || null
}

function tabFieldsFromHtml(html, tabId) {
  const escapedId = String(tabId).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
  const tab = String(html).match(
    new RegExp(`<div\\b[^>]*\\bid=["']${escapedId}["'][^>]*>[\\s\\S]*?<table\\b[^>]*>([\\s\\S]*?)<\\/table>`, "i")
  )
  return detailFieldsFromHtml(tab?.[1] || "")
}
...
function parseEplanApplicationHtml(html, expectedReference) {
  ...
  const decisionFields = tabFieldsFromHtml(html, "Decision")
  const appealFields = tabFieldsFromHtml(html, "Appeal")
  ...
  return {
    ok: true,
    fileNumber,
    status: textFor(fields, "planning status"),
    ...
    decision_date: valueFor(decisionFields, "decision date") || valueFor(fields, "decision date"),
    decision_text:
      textFor(decisionFields, "decision type") ||
      textFor(fields, "decision type") ||
      textFor(decisionFields, "decision description"),
    final_grant_date: valueFor(decisionFields, "grant date"),
    withdrawal_date: valueFor(fields, "withdrawn date"),
    appeal_lodged_date: valueFor(fields, "appeal date"),
    appeal_decision_date: valueFor(appealFields, "decision date"),
