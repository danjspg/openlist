...
function applicationUpdates(app, state, changedFields) {
...
function createPlanningAlertWatcher({
  ...
  async function fetchCorkApplication(app) {
    ...
    const result = await fetchJson(`${CORK_DETAIL_URL}/${Number(app.source_application_id)}`, app.reference, {
      "x-client": CORK_AUTHORITY_CODE,
  ...
  async function fetchSourceState(app) {
    ...
    if (app.local_authority_code === CORK_AUTHORITY_CODE) {
    ...
    let query = supabase
      ...
      .select(`
        ...
        planning_applications(
          id,reference,local_authority_code,source_application_id,status,normalized_status,
  ...
  async function processApplication(app) {
    ...
    const changedFields = changedWatchFields(watch.state || {}, state)
    const updates = applicationUpdates(app, state, changedFields)
    ...
    if (changedFields.length > 0) await queueRevalidation(app.id, now)
