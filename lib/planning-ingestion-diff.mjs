...
/** @param {any} supabase @param {Record<string, any>[]} records @param {{ authorityCode: string, from?: string, to?: string, pageSize?: number, preserveUnobservedFields?: string[] }} options */
async function filterChangedPlanningRecords(
  supabase,
  records,
  { authorityCode, from, to, pageSize = 1000, preserveUnobservedFields = [] }
) {
  ...
  const changedRecords = records.map((record) => {
    ...
    if (!existing) return { record, changed: true }
    ...
    return {
      record: preserved,
      changed: planningRecordsDifferWithOptions(existing, record, { preserveUnobservedFields }),
    }
  }).filter((entry) => entry.changed).map((entry) => entry.record)
