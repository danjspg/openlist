...
export function extractPlanningScaleSignals(proposal) {
  ...
  const residentialUnits = maximumCount(
    text,
...
export function classifyPlanningNotability(application, options = {}) {
  ...
  if (scale.residentialUnits >= thresholds.residentialUnits) {
    add("residential-large", "residential.unit-threshold", `${scale.residentialUnits} residential units (threshold ${thresholds.residentialUnits})`, 2)
  }
