...
export function classifyPlanningNotability(application, options = {}) {
  ...
  const primarySubstationDevelopment = /\b(?:construction|construct|development|provision|erection|upgrade|extension)\b.{0,60}\b(?:electricity|electrical|esb)?\s*substation\b|\b(?:electricity|electrical|esb)?\s*substation\b.{0,60}\b(?:construction|development|upgrade|extension)\b/
  if (strongGridInfrastructure.test(text) || primarySubstationDevelopment.test(text)) {
    add("infrastructure", "infrastructure.electricity-grid", "Electricity grid or transmission infrastructure", 2)
  }
  ...
  if (portInfrastructure.test(text)) add("transport", "transport.port", "Port or harbour infrastructure", 2)
