...
export const PUBLIC_PLANNING_CATEGORY_SLUGS = Object.freeze([
  "padel",
  "residential-development",
  "large-residential",
  "wind-farms",
...
export function extractPlanningScaleSignals(proposal) {
  ...
  const residentialUnits = explicitResidentialUnits || componentResidentialUnits
...
export function classifyPlanningNotability(application, options = {}) {
  ...
  const publicCategoryReasons = new Map()
  let strongest = 0

  const add = (category, rule, reason, strength = 1, publicCategory = null) => {
    matchedRules.push(rule)
    if (!categoryReasons.has(category)) categoryReasons.set(category, reason)
    if (publicCategory && !publicCategoryReasons.has(publicCategory)) {
      publicCategoryReasons.set(publicCategory, reason)
    }
    strongest = Math.max(strongest, strength)
  }
  const match = (pattern, category, rule, reason, strength = 2, publicCategory = null) => {
    if (pattern.test(text)) add(category, rule, reason, strength, publicCategory)
  }

  const padelMention = /\bpadel\b/.test(text)
  const padelPlaceNameOnly = /\bpadel\s+(?:road|street|avenue|lane|drive|close|park|estate)\b/.test(text)
  const padelDevelopmentContext = /\bpadel tennis\b|\bpadel\b.{0,30}\b(?:clubs?|courts?|facilit(?:y|ies)|cent(?:re|er)s?|developments?)\b|\b(?:clubs?|courts?|facilit(?:y|ies)|cent(?:re|er)s?|developments?)\b.{0,30}\bpadel\b/.test(text)
  if (padelMention && (!padelPlaceNameOnly || padelDevelopmentContext)) {
    add("padel", "leisure.padel", "Padel court, club or facility development", 2, "padel")
  }
  match(/\bwind farm\b|\b(?:\d+|six|seven|eight|nine|ten|eleven|twelve)[ -](?:wind )?turbines?\b|\bwind turbine\b/, "energy", "energy.wind", "Wind energy development", 2, "wind-farms")
  match(/\bsolar farm\b|\bsolar energy (?:farm|development|facility)\b|\bphotovoltaic (?:farm|energy development|array)\b/, "energy", "energy.solar", "Solar energy development", 2, "solar-energy")
  match(/\bbattery energy storage(?: system| facility)?\b|\bbess\b|\bgrid[- ]scale battery\b/, "energy", "energy.bess", "Battery energy storage development", 2, "battery-storage")

  // Treat substations as infrastructure only when the electrical works are a
  // substantive purpose of the application. Incidental ESB kiosks/substations
  // attached to housing, retail or hospitality schemes are not a separate
  // infrastructure development.
  const strongGridInfrastructure = /\b(?:grid connection|grid infrastructure|transmission (?:line|infrastructure|station)|high voltage electricity infrastructure|(?:110|220|400)\s*k?v\s+(?:electricity\s+|electrical\s+|esb\s+)?substation)\b/
  const primarySubstationDevelopment = /\b(?:construction|construct|development|provision|erection|upgrade|extension)\b\s+(?:of|to)?\s*(?:an?|the)?\s*(?:new|existing|proposed)?\s*(?:electricity|electrical|esb)?\s*substation\b|\b(?:electricity|electrical|esb)?\s*substation\b\s+(?:construction|development|upgrade|extension)\b/
  if (strongGridInfrastructure.test(text) || primarySubstationDevelopment.test(text)) {
    add("infrastructure", "infrastructure.electricity-grid", "Electricity grid or transmission infrastructure", 2, "infrastructure")
  }
  match(/\bdata cent(?:re|er)s?\b|\bdata storage campus\b/, "data-centre", "infrastructure.data-centre", "Data centre development", 2, "data-centres")
  ...
  if (/\b(?:telecommunications?|telecoms?)\b/.test(text) && /\b(?:mast|tower|monopole|base station|compound)\b/.test(text)) {
    ...
    add("infrastructure", "infrastructure.telecoms", "Significant telecommunications infrastructure", height >= 15 || SUBSTANTIVE_DEVELOPMENT.test(text) ? 2 : 1, "infrastructure")
  }
  match(/\brailway station\b|\brail infrastructure\b|\brailway (?:line|depot|platform|terminal)\b/, "transport", "transport.rail", "Railway station or rail infrastructure", 2, "transport")
  match(/\b(?:airport|aerodrome)\b.*\b(?:runway|terminal|hangar|infrastructure|extension|development)\b|\b(?:runway|airport terminal)\b/, "transport", "transport.airport", "Airport infrastructure", 2, "transport")

  // Do not let domestic "car-port" satisfy a port-infrastructure rule. A real
  // port token must begin at whitespace/punctuation (or the start of text).
  const portInfrastructure = /(?:^|[\s,(])port\b.{0,120}\b(?:infrastructure|terminal|quay|pier|berth|development|extension)\b|\b(?:harbour|harbor)\b.{0,120}\b(?:infrastructure|terminal|quay|pier|berth|development|extension)\b|\b(?:port terminal|harbour infrastructure|harbor infrastructure)\b/
  if (portInfrastructure.test(text)) add("transport", "transport.port", "Port or harbour infrastructure", 2, "transport")
  match(/\b(?:motorway|dual carriageway|national primary road|national secondary road|major road infrastructure|road improvement scheme)\b/, "transport", "transport.major-road", "Motorway or major road infrastructure", 2, "transport")
  match(/\b(?:waste treatment|waste transfer|waste recovery|waste processing|landfill|incinerator|incineration|materials recovery facility|recycling centre)\b/, "waste", "waste.facility", "Waste treatment, transfer, recovery or disposal facility", 2, "waste-recycling")
  match(/\b(?:quarry|quarrying|mining|mine development|mineral extraction|rock extraction|sand and gravel extraction)\b/, "quarry", "industrial.extraction", "Quarry, mining or extraction development", 2, "quarrying")
  ...
  if (industrialFacility.test(text)) {
    ...
    add("industrial", "industrial.major-facility", "Factory, manufacturing plant or industrial facility", strength, "industrial-logistics")
  }
  ...
  if (retailDevelopment.test(text) || applicantOperator && /\b(?:store|foodstore|supermarket|restaurant|cafe|coffee shop|drive[- ]?(?:through|thru))\b/.test(proposal) || proposalOperatorUse) {
    add("retail", applicantOperator || proposalOperatorUse ? "retail.recognisable-operator" : "retail.anchor", "Significant or recognisable retail development", retailDevelopment.test(text) ? 2 : 1, "retail")
  }
  if (/\bhotel\b/.test(text)) {
    add("hospitality", "hospitality.hotel", "Hotel development", SUBSTANTIVE_DEVELOPMENT.test(text) || SCALE_TERMS.test(text) ? 2 : 1, "hotels-restaurants")
  }
  if (/\bdrive[- ]?(?:through|thru)\b/.test(text)) add("hospitality", "hospitality.drive-through", "Drive-through commercial development", 2, "hotels-restaurants")
  ...
  if (campusDevelopment.test(text) || logisticsDevelopment.test(text)) {
    add(
      ...
      2,
      /industrial estate|logistics|distribution/.test(text) ? "industrial-logistics" : null
    )
  }
  ...
  if (warehouse && (scale.floorAreaSqm >= thresholds.substantialFloorAreaSqm && !/\bdemolition\b.{0,60}\bwarehouses?\b/.test(text) || warehouseDevelopment.test(text))) {
    add("industrial", "industrial.substantial-warehouse", "Substantial warehouse development", 2, "industrial-logistics")
  }
  ...
  if (restaurant && (RECOGNISABLE_OPERATORS.test(operatorText) || /\b(?:major|large|drive[- ]?(?:through|thru))\b/.test(text) || scale.floorAreaSqm >= 500)) {
    add("hospitality", "hospitality.major-restaurant", "Major or recognisable restaurant development", /drive|major|large/.test(text) ? 2 : 1, "hotels-restaurants")
  }
  ...
  if (scale.residentialUnits >= thresholds.largeResidentialUnits) {
    add("residential-large", "residential.unit-threshold", `${scale.residentialUnits} residential units (threshold ${thresholds.largeResidentialUnits})`, 2, "large-residential")
  } else if (scale.residentialUnits >= thresholds.residentialUnits) {
    add("residential", "residential.unit-threshold", `${scale.residentialUnits} residential units (threshold ${thresholds.residentialUnits})`, 2, "residential-development")
  }
  if (scale.studentAccommodationUnits >= thresholds.studentAccommodationUnits) {
    add("student-accommodation", "residential.student-threshold", `${scale.studentAccommodationUnits} student accommodation units/bedspaces (threshold ${thresholds.studentAccommodationUnits})`, 2, "student-accommodation")
  }

  const excluded = exclusions.length > 0 && strongest < 2
  const categories = excluded ? [] : [...categoryReasons.keys()]
  const publicCategories = excluded ? [] : [...publicCategoryReasons.keys()]
