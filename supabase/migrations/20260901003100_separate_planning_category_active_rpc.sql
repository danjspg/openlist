-- Avoid overloading the public category RPC with an optional sixth argument.
-- PostgREST cannot disambiguate a five-argument call when both signatures exist.
drop function if exists public.openlist_planning_public_category_page(text, boolean, text, int, int, boolean);
...
as $$
  ...
  select jsonb_build_object(
    ...
    'authorityCounts',coalesce((select jsonb_agg(jsonb_build_object('code',a.local_authority_code,'count',a.category_count) order by a.category_count desc,a.local_authority_code) from (select local_authority_code,count(*) category_count from eligible group by local_authority_code) a),'[]'::jsonb)
  );
$$;

revoke all on function public.openlist_planning_public_category_page_active(text, boolean, text, int, int, boolean) from public;
grant execute on function public.openlist_planning_public_category_page_active(text, boolean, text, int, int, boolean) to anon, authenticated, service_role;
