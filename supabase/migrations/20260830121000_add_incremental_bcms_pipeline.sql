...
create table public.bcms_pipeline_checkpoints (
  stage text primary key check (stage in ('acquisition_append','acquisition_audit','normalization','matching','notable_catchup')),
...
declare inserted_count int; input_count int := jsonb_array_length(coalesce(p_rows, '[]'::jsonb));
begin
  insert into public.bcms_raw_record_versions(source_resource_id, source_record_id, content_hash, payload, acquisition_run_id)
  ...
  values(case when p_mode='audit' then 'acquisition_audit' else 'acquisition_append' end,p_end_cursor,p_source_freshness_at,now(),null,jsonb_build_object('input',input_count,'newOrChanged',inserted_count,'unchanged',input_count-inserted_count))
  ...
  return jsonb_build_object('inputRows',input_count,'newOrChangedRows',inserted_count,'unchangedRows',input_count-inserted_count,'cursor',p_end_cursor);
end $$;
...
begin
  for item in select * from public.bcms_raw_record_versions where processed_at is null order by acquired_at,id limit greatest(1,least(coalesce(p_limit,200),500)) for update skip locked loop
    begin
      ...
      update public.bcms_raw_record_versions set processed_at=now(),processing_attempts=processing_attempts+1,processing_error=null where id=item.id;
      processed_count := processed_count + 1;
    exception when others then
      update public.bcms_raw_record_versions set processing_attempts=processing_attempts+1,processing_error=sqlerrm where id=item.id;
...
returns jsonb language plpgsql security definer set search_path=public,pg_catalog set statement_timeout='15s' as $$
declare item record; candidate_count int; app_id uuid; notice_row public.building_control_notices%rowtype; linked int:=0; noop int:=0; ambiguous int:=0; unmatched int:=0; failures int:=0; checked_notable int:=0; did_link boolean;
begin
  for item in select * from public.bcms_match_queue where state in('pending','failed') and available_at<=now() order by priority,id limit greatest(1,least(coalesce(p_limit,200),500)) for update skip locked loop
    begin
      ...
      if notice_row.id is null or app_id is null then update public.bcms_match_queue set state='unmatched',updated_at=now() where id=item.id; unmatched:=unmatched+1;
        ...
        update public.planning_applications p set construction_status=derived.status,construction_evidence_date=derived.evidence_date,construction_evidence_source='NBCO/BCMS open data',construction_evidence_detail=derived.detail
        from (select case when bool_and(coalesce(n.total_phases,1)=1) and bool_and(n.completion_certificate_count>0) and coalesce(sum(n.completion_units),0)>=coalesce(nullif(sn.extracted_residential_units,0),2147483647) then 'completed' when bool_or(n.commencement_date is not null) then 'commenced' end status,max(coalesce((select max(r.completion_certificate_validated_at) from public.building_control_records r where r.source_resource_id=n.source_resource_id and r.building_control_authority_code=n.building_control_authority_code and r.cn_number=n.cn_number),n.commencement_date)) evidence_date,case when count(*)>1 or bool_or(coalesce(n.total_phases,1)>1) then 'Matched phased building-control records' else 'Matched building-control notice '||min(n.cn_number) end detail from public.planning_building_control_links l join public.building_control_notices n on n.id=l.notice_id left join public.planning_seo_notable sn on sn.application_id=l.planning_application_id where l.planning_application_id=app_id group by sn.extracted_residential_units) derived where p.id=app_id and derived.status is not null;
...
revoke all on function public.openlist_bcms_store_acquired_rows(text,jsonb,uuid,text,text,timestamptz),public.openlist_bcms_process_raw_batch(int),public.openlist_bcms_enqueue_notable_catchup(uuid,int),public.openlist_bcms_match_batch(int),public.openlist_bcms_integrity_report() from public,anon,authenticated;
grant execute on function public.openlist_bcms_store_acquired_rows(text,jsonb,uuid,text,text,timestamptz),public.openlist_bcms_process_raw_batch(int),public.openlist_bcms_enqueue_notable_catchup(uuid,int),public.openlist_bcms_match_batch(int),public.openlist_bcms_integrity_report() to service_role;
