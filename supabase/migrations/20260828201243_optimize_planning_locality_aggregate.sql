-- Keep locality dashboards on the existing authority/locality covering index.
-- Locality page inputs are canonical labels produced by openlist_planning_locality(),
-- so the previous location ILIKE + UNION scan was redundant and substantially more
...
type_counts as (
  select
    public.openlist_planning_application_type_label(
...
comment on function public.openlist_planning_area_aggregate(text, text) is
  'Returns dashboard aggregates for a canonical authority locality using the authority/locality covering index.';
