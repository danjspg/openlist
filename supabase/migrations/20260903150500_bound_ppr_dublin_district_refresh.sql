create or replace function public.openlist_refresh_ppr_dublin_district_insight(p_market_slug text)
returns void
language plpgsql
...
declare
  ...
  v_label text;
begin
  select prefix, market_label
  into v_prefix, v_label
  from (values
  ...
  ) as districts(prefix, market_slug, market_label)
  where market_slug = p_market_slug;

  if v_prefix is null then
    raise exception 'Unknown Dublin district market slug: %', p_market_slug;
  end if;

  delete from public.ppr_market_insights
  where range_key = 'last-year' and market_slug = p_market_slug;
  ...
  with bounds as (
    ...
    select s.date_of_sale, s.price_eur::numeric as price_eur, s.is_new_dwelling
    from public.ppr_sales s
    cross join bounds b
    where upper(s.eircode_prefix) = v_prefix
      and s.date_of_sale between b.baseline_start and b.current_end
  ), agg as (
    select
  ...
  select
    'last-year', p_market_slug, v_label, 'dublin_district', 'Dublin', current_count,
    ...
    case when current_count >= 24 and previous_count >= 24 and previous_median > 0 then ((current_median - previous_median) / previous_median) * 100 end,
    case when baseline_count >= 24 and baseline_median > 0 then ((current_median - baseline_median) / baseline_median) * 100 end,
    'Last 12 months', current_count, 'Previous 12 months', previous_count,
    case when current_count >= 24 and previous_count >= 24 and previous_count > 0 then ((current_count - previous_count)::numeric / previous_count) * 100 end,
    ...
    case when new_count >= 8 and second_count >= 8 and second_median > 0 then ((new_median - second_median) / second_median) * 100 end,
    now()
  from agg;
end;
$function$;

revoke all on function public.openlist_refresh_ppr_dublin_district_insight(text) from public, anon, authenticated;
grant execute on function public.openlist_refresh_ppr_dublin_district_insight(text) to service_role;
