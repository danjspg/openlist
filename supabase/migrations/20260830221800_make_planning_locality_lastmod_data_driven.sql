...
begin
  with search_perf as (
    ...
    select m.id,
      ...
      + case when coalesce((m.evidence->>'applicationCount')::integer,0) >= 50 then 12 else 0 end as priority_score,
  ...
  set seo_priority_score = r.priority_score,
      seo_tier = case when r.rn <= v_target or r.notable_count > 0 or r.clicks > 0 then 'priority' else 'expanded' end
  from ranked r
  where m.id = r.id;
    ...
    coalesce(
      (select max(p.updated_at)
       ...
       where p.local_authority_code = m.authority_code
         and public.openlist_locality_slug(public.openlist_planning_locality(p.location, p.ward, p.local_authority_code)) = m.locality_slug),
