-- Public development-type browse pages can be filtered to currently active applications.
-- Active lifecycle semantics match the shared Planning status model.
...
as $$
  ...
  select jsonb_build_object(
    ...
    'activeCount', (
      select count(*) from authority_filtered
...
revoke all on function public.openlist_planning_public_category_page(text, boolean, text, int, int, boolean) from public;
grant execute on function public.openlist_planning_public_category_page(text, boolean, text, int, int, boolean)
