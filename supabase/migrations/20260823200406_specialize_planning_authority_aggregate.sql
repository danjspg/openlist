create or replace function public.openlist_planning_authority_aggregate(
...
with filtered as materialized (
  select
    ...
    p.grid_easting,
    p.grid_northing,
    public.openlist_planning_locality(p.location, p.ward, p.local_authority_code) as area_label
...
as $function$
begin
  if p_authority_code is not null
     and nullif(trim(p_q), '') is null
     and nullif(trim(p_area), '') is null
     and nullif(trim(p_status), '') is null
     and nullif(trim(p_application_type), '') is null then
    return public.openlist_planning_authority_aggregate(p_authority_code);
  end if;
