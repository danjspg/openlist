...
export const metadata: Metadata = {
  ...
  metadataBase: new URL(process.env.NEXT_PUBLIC_SITE_URL || "https://www.openlist.ie"),
  icons: {
    icon: [{ url: "/favicon-v2.ico?v=2", sizes: "16x16 32x32", type: "image/x-icon" }],
    shortcut: "/favicon-v2.ico?v=2",
    apple: [{ url: "/apple-icon-v2.png?v=2", sizes: "180x180", type: "image/png" }],
  },
}
            ...
            <div className="mx-auto max-w-6xl px-4 sm:px-6">
              <div className="flex flex-wrap items-center justify-between gap-x-4 py-3 sm:py-4 md:flex-nowrap md:py-4">
                <Link href="/" className="flex shrink-0 items-center">
                  <Image src="/logo-small.png" alt="OpenList" width={180} height={180} className="h-[45px] w-[45px] sm:hidden" priority />
                  <Image src="/logo-v2.png" alt="OpenList" width={450} height={131} className="hidden h-auto w-56 sm:block md:w-60 lg:w-64" priority />
                </Link>
                <Nav />
              </div>
            </div>
              ...
              <div className="grid gap-10 lg:grid-cols-[1.05fr_1.45fr] lg:gap-14">
                <div>
                  ...
                  <p className="mt-4 max-w-md text-sm leading-6 text-stone-600">
                    Irish planning applications and sold-price data, organised to make local research faster and easier to understand.
                  </p>
                  <p className="mt-4 max-w-md text-xs leading-5 text-stone-400">
                    Public-data research only. Always check the relevant official source before relying on a record.
                  </p>
                </div>
                ...
                <nav aria-label="Footer" className="grid grid-cols-2 gap-x-7 gap-y-9 text-sm text-stone-600 sm:grid-cols-4">
                  ...
                  <FooterLinkGroup title="About">
                    <Link href="/about" className="transition hover:text-stone-900">About OpenList</Link>
                    <Link href="/data-sources" className="transition hover:text-stone-900">Data sources</Link>
...
function FooterLinkGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h2 className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-400">{title}</h2>
