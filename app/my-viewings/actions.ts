...
export async function createViewing(formData: FormData) {
  ...
  const contactName = getRequired(formData, "contactName", "Property contact name")
  const contactEmail = normaliseEmail(getRequired(formData, "contactEmail", "Property contact email"))
