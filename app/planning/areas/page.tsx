...
export default async function PlanningAreasPage() {
  ...
  const entries: AreaEntry[] = memberships.map((membership) => {
    ...
    return {
      ...
      applicationCount: safeCount(membership.evidence?.applicationCount),
      latestRegistrationDate: membership.evidence?.latestRegistrationDate ?? null,
    }
  })
  ...
  const groups: CountyGroup[] = [...groupedMap.entries()]
    .map(([county, areas]) => ({
      ...
      activity: areas.reduce((sum, area) => sum + area.applicationCount, 0),
    }))
    .sort((left, right) =>
