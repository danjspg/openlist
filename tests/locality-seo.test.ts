...
test("locality routes, sitemaps, robots and migration preserve permanent pages", async () => {
  ...
  assert.match(soldSitemap, /dynamic = "force-dynamic"/)
  assert.match(soldSitemap, /soldPricesLocalities/)
  assert.match(planningSitemap, /dynamic = "force-dynamic"/)
  assert.match(planningSitemap, /planningLocalitiesPriority/)
  assert.doesNotMatch(`${soldSitemap}\n${planningSitemap}`, /getLocalitySitemap|@\/lib\/locality-seo/)
