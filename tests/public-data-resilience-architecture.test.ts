...
test("filtered Planning performs only its bounded result query", async () => {
  ...
  assert.match(planning, /\.limit\(26\)/)
  assert.match(planning, /const hasMore = rows\.length > 25/)
  assert.doesNotMatch(planning, /select\(PLANNING_APPLICATION_SELECT, \{ count: "exact" \}\)/)
