...
async function upsertCases(rows) {
  for (let offset = 0; offset < rows.length; offset += UPSERT_SIZE) {
    ...
    const { error } = await supabase.from("planning_appeal_cases").upsert(batch, { onConflict: "source_object_id" })
...
async function main() {
  ...
  try {
    ...
    await updateSourceState({
      ...
      last_error: null,
      metadata: { enrichment, rebuild: rebuilt, maxRecordCount: metadata.maxRecordCount },
    })
    console.log(JSON.stringify({ sourceRecords: rows.length, enrichment, rebuild: rebuilt }, null, 2))
  } catch (error) {
